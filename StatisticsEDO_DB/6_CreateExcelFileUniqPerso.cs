...
{
    static class CreateExcelFileUniqPerso
    {
        ...
        public static void CreateNewExcelFile(Dictionary<string, DataFromPersoDB> dictionaryUnikRegNomPersoALL)
        {
            ...
            try
            {
                ...
                workSheet.Name = "УникРегНомераPerso";
                ...
                workSheet.Cells[i, 10] = "Результат проверки";
                workSheet.Cells[i, 11] = "Дата проверки";
                workSheet.Cells[i, 12] = "Количество ЗЛ за отч. период (принято, не проверено)";
                //workSheet.Cells[i, 12] = "Количество ЗЛ в файле";
                //workSheet.Cells[i, 13] = "ЗЛ принято";
                //workSheet.Cells[i, 14] = "ЗЛ не принято";
                workSheet.Cells[i, 13] = "Статус квитанции";
                workSheet.Cells[i, 14] = "Специалист (последний принятый документ)";
                workSheet.Cells[i, 15] = "Способ представления (в отчетном периоде)";
                workSheet.Cells[i, 16] = "Куратор";
                workSheet.Cells[i, 17] = "УП по данным УПФР";                
                workSheet.Cells[i, 18] = "Категория";
                workSheet.Cells[i, 19] = "ИНН";
                workSheet.Cells[i, 20] = "Дата постановки в ПФР";
                workSheet.Cells[i, 21] = "Дата постановки в РО";
                workSheet.Cells[i, 22] = "Дата снятия в РО";
                workSheet.Cells[i, 23] = "Наличие ДОП формы";
                workSheet.Cells[i, 24] = "Количество ЗЛ в ДОП формах";
                workSheet.Cells[i, 25] = "ЗЛ принято (ДОП формы)";
                workSheet.Cells[i, 26] = "ЗЛ не принято (ДОП формы)";
                workSheet.Cells[i, 27] = "Наличие ОТМН формы";
                workSheet.Cells[i, 28] = "Количество ЗЛ в ОТМН формах";
                workSheet.Cells[i, 29] = "ЗЛ принято (ОТМН формы)";
                workSheet.Cells[i, 30] = "ЗЛ не принято (ОТМН формы)";
                workSheet.Cells[i, 31] = "Tout_прошлый кол-во ЗЛ";
                workSheet.Cells[i, 32] = "Tout_текущий кол-во ЗЛ";                
                ...
                foreach (var row in dictionaryUnikRegNomPersoALL)
                {
                    ...
                    //         + dopFormAvailability + ";" + otmnFormAvailability + ";" + toutOldCountZL + ";" + toutNewCountZL + ";";
                    ...
                    workSheet.Cells[j, 10] = row.Value.resultat;
                    workSheet.Cells[j, 11] = row.Value.dataProverki;
                    workSheet.Cells[j, 12] = row.Value.kolZLBDPerso;
                    //workSheet.Cells[j, 12] = row.Value.kolZL;
                    //workSheet.Cells[j, 13] = row.Value.kolZLgood;
                    //workSheet.Cells[j, 14] = row.Value.kolZLbad;
                    workSheet.Cells[j, 13] = row.Value.statusKvitanc;
                    workSheet.Cells[j, 14] = row.Value.spec;
                    workSheet.Cells[j, 15] = row.Value.specChanged;                    
                    workSheet.Cells[j, 16] = row.Value.curator;
                    workSheet.Cells[j, 17] = row.Value.UP;
                    workSheet.Cells[j, 18] = row.Value.kategory;
                    workSheet.Cells[j, 19] = row.Value.inn;
                    workSheet.Cells[j, 20] = row.Value.dataPostPFR;
                    workSheet.Cells[j, 21] = row.Value.dataPostRO;
                    workSheet.Cells[j, 22] = row.Value.dataSnyatRO;
                    workSheet.Cells[j, 23] = row.Value.dopFormAvailability;
                    workSheet.Cells[j, 24] = row.Value.dopFormKolZL;
                    workSheet.Cells[j, 25] = row.Value.dopFormKolZLgood;
                    workSheet.Cells[j, 26] = row.Value.dopFormKolZLbad;
                    workSheet.Cells[j, 27] = row.Value.otmnFormAvailability;
                    workSheet.Cells[j, 28] = row.Value.otmnFormKolZL;
                    workSheet.Cells[j, 29] = row.Value.otmnFormKolZLgood;
                    workSheet.Cells[j, 30] = row.Value.otmnFormKolZLbad;
                    workSheet.Cells[j, 31] = row.Value.toutOldCountZL;
                    workSheet.Cells[j, 32] = row.Value.toutNewCountZL;                            
                ...
                //TODO: Внимание! При изменении количества ячеек с данными изменить значение диапазона
                Excel.Range rangeData = workSheet.Range[workSheet.Cells[2, 1], workSheet.Cells[dictionaryUnikRegNomPersoALL.Count() + 1, 32]];
                //шрифт для захваченного диапазона
                ...
                //авто ширина и авто высота
                //TODO: Внимание! При изменении количества ячеек с данными изменить значение диапазона
                Excel.Range rangeData1 = workSheet.Range[workSheet.Cells[2, 1], workSheet.Cells[dictionaryUnikRegNomPersoALL.Count() + 1, 4]];
                rangeData1.EntireColumn.AutoFit();
                rangeData1.EntireRow.AutoFit();
                Excel.Range rangeData2 = workSheet.Range[workSheet.Cells[2, 6], workSheet.Cells[dictionaryUnikRegNomPersoALL.Count() + 1, 32]];
...
End Sub
                */
                ...
                try
                {
                    ...
                    string fileNameExcel = @"C:\_Out" + @"\" + @"УникРегНомерPerso_Общий_реестр_" + DateTime.Now.ToShortDateString() + "_.xlsx";
